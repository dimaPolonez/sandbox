...
function convert(input, source, target) {
    ...
    arrInput.forEach((value) => {
        count = (count * source.length) + source.indexOf(value);
    });
    
    while (count > 0) {
        output = target[count % target.length] + output;
        count = Math.floor(count / target.length);
    }
    
    return output ? output : target[0];
}